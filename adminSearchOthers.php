...
<body>
	...
	<select name="sid">
		...
		<?php
			...
			$stmt = $conn->query("SELECT sid,sName FROM oneshopy_undolt.student where banned=0;");
	...
	<select name="mid">
		...
		<?php
			...
			$stmt = $conn->query("SELECT mName,mid FROM oneshopy_undolt.manager where approved='Yes' and banned=0;");
			while ($row=$stmt->fetch(PDO::FETCH_ASSOC)){
					$mid = $row['mid'];
	...
	<select name="tid">
		...
		<?php
			...
			$stmt = $conn->query("SELECT tid,tName FROM oneshopy_undolt.tutor where approved ='Approve' and banned=0;");
			while ($row=$stmt->fetch(PDO::FETCH_ASSOC)){
					$tid = $row['tid'];
	...
	<select name="qid">
		...
		<?php
			...
			$stmt = $conn->query("SELECT qid,subject,sid FROM oneshopy_undolt.assignment where aStatus<>'Pending';");
			while ($row=$stmt->fetch(PDO::FETCH_ASSOC)){
					$qid = $row['qid'];
...
<?php 
		...
		if (isset($_POST['mid']) && $_POST['mid']!='none') {
			$mid = $_POST['mid'];
			$stmt5 = $conn->query("SELECT * FROM oneshopy_undolt.manager WHERE mid=$mid;");
