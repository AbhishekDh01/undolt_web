<?php
	...
	if (isset($_POST['removeStudent'])) {
		...
		$stmtss = $conn->query("SELECT * FROM oneshopy_undolt.student where sid = $sid;");
		$rowss=$stmtss->fetch(PDO::FETCH_ASSOC);

		$sql15 = "update oneshopy_undolt.student set banned = 1 where sid = $sid;";
		$stmt15 = $conn->prepare($sql15);
		...
		error_reporting( E_ALL );
         ...
         $message = "
					<html>
					<head>
					<title>Student Blocked</title>
					</head>
					<body>
					<h2>$name you have been blocked for not following rules</h2>
	...
	if (isset($_POST['removeTutor'])) {
		...
		$stmtss = $conn->query("SELECT * FROM oneshopy_undolt.tutor where tid = $tid;");
		$rowss=$stmtss->fetch(PDO::FETCH_ASSOC);

		$sql16 = "update oneshopy_undolt.tutor set banned = 1 where tid = $tid;";
	...
	if (isset($_POST['removeManager'])) {
		...
		$stmtss = $conn->query("SELECT * FROM oneshopy_undolt.manager where mid = $mid;");
		$rowss=$stmtss->fetch(PDO::FETCH_ASSOC);

		$sql17 = "update oneshopy_undolt.manager set banned = 1 where mid = $mid;";
